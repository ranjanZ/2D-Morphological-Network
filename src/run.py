...
def train_all_models(num_epochs=1000):
    ...
    # train CNN
    model_cnn = model_CNN()
    model_cnn.compile(loss=loss_new, optimizer="adam",
                      metrics=[SSIM(kernel_size=100)])
    model_cnn.fit_generator(gen, epochs=num_epochs)
    model_cnn.save_weights("../models/model_cnn.h5")

    # train path1
    path1 = model_path1()
    path1.compile(loss=loss_new, optimizer="adam",
                  metrics=[SSIM(kernel_size=100)])
    ...
    # train path2
    path2 = model_path2()
    path2.compile(loss=loss_new, optimizer="adam",
                  metrics=[SSIM(kernel_size=100)])
    ...
    morphoN.compile(loss=loss_new, optimizer="adam",
                    metrics=[SSIM(kernel_size=100)])
    ...
    morphoN_small.compile(loss=loss_new, optimizer="adam",
                          metrics=[SSIM(kernel_size=100)])
...
def save_all_results(output_path="../data/train_data/results/"):
    ...
    t2_cnn, t2_path1, t2_path2, t2_path12, t2_small = D[0].predict(
        t1), D[1].predict(t1), D[2].predict(t1), D[3].predict(t1), D[4].predict(t1)
    ...
    for j in range(t2.shape[0]):
        sys.stdout.write("\r %d/%d " % (j, t2.shape[0]))
        sys.stdout.flush()
        """
...
def main(input_dir="../data/input_images/", output_dir="../data/output/"):
    ...
    D_name = {-1: "input_", 0: "cnn", 1: "path1_",
              2: "path2_", 3: "morpho_net_", 4: "small_morpho_net_"}
    ...
    for file_name in os.listdir(input_dir):
        ...
        if(True):
            plt.imsave(output_dir+file_name.split(".")
                       [0]+D_name[-1]+"."+file_name.split(".")[1], img_in[:, :], cmap="gray")
            ...
            for i in [0, 1, 2, 3, 4]:
                ...
                plt.imsave(output_dir+file_name.split(".")
                           [0]+D_name[i]+"."+file_name.split(".")[1], t3[:, :, 0], cmap="gray")
        else:
